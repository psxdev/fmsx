...
int SaveRPL(const char *FileName)
{
  ...
  /* Open file */
  F = ps4LinkOpen(FileName,O_RDWR|O_CREAT,0777);
  if(!F) return(0);
  ...
  /* Write initial state */
  if(ps4LinkWrite(F,RPLData[J].State,RPLData[J].StateSize)!=RPLData[J].StateSize)
  { ps4LinkClose(F);return(0); }
...
int LoadRPL(const char *FileName)
{
  ...
  if(ps4LinkRead(F,Header,sizeof(Header))!=sizeof(Header)) { ps4LinkClose(F);return(0); }
  if(memcmp(Header,"RPL\032\001",5)) { ps4LinkClose(F);return(0); }
  ...
  /* Read state */
  if(ps4LinkRead(F,P,J)!=J) { ps4LinkClose(F);free(P);return(0); }
