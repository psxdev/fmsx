...
#if defined(UIQ) || defined(S60) || defined(UNIX) || defined(ORBIS) || defined(MAEMO) || defined(MEEGO) || defined(ANDROID)
#include <dirent.h>
#else
#include <direct.h>
#endif
...
byte *DSKLoad(const char *Name,byte *Dsk)
{
  ...
  if(F<0) { if(!Dsk) free(Dsk1);return(0); }

  /* Read data */
  if(ps4LinkRead(F,Dsk1,DSK_DISK_SIZE)!=DSK_DISK_SIZE)
  { if(!Dsk) free(Dsk1);ps4LinkClose(F);return(0); }
...
const byte *DSKSave(const char *Name,const byte *Dsk)
{
  ...
  /* Assume <Name> to be a disk image file */
  F=ps4LinkOpen(Name,O_RDWR|O_CREAT ,0777);
  if(!F) return(0);

  /* Write data */
  if(ps4LinkWrite(F,Dsk,DSK_DISK_SIZE)!=DSK_DISK_SIZE)         
