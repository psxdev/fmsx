...
int LoadFDI(FDIDisk *D,const char *FileName,int Format)
{
  ...
  J=ps4LinkLseek(F,0,SEEK_END);
  if(J<0){ ps4LinkClose(F);return(0); }
  ...
  switch(Format)
  {
    case FMT_FDI: /* If .FDI format... */
      ...
      if(!(P=(byte *)malloc(J))) { ps4LinkClose(F);return(0); }
      if(ps4LinkRead(F,P,J)!=J)      { free(P);ps4LinkClose(F);return(0); }
      /* Verify .FDI format tag */
      if(memcmp(P,"FDI",3))      { free(P);ps4LinkClose(F);return(0); }
    ...
    case FMT_DSK: /* If .DSK format... */
      /* Read header */
      if(ps4LinkRead(F,Buf,32)!=32) { ps4LinkClose(F);return(0); }
      /* Check magic number */
      if((Buf[0]!=0xE9)&&(Buf[0]!=0xEB)) { ps4LinkClose(F);return(0); }
      /* Check media descriptor */
      if(Buf[21]<0xF8) { ps4LinkClose(F);return(0); }
    ...
    case FMT_CPCDSK: /* If Amstrad CPC .DSK format... */
      ...
      if(ps4LinkRead(F,Buf,256)!=256) { ps4LinkClose(F);return(0); }
      /* Check magic string */
...
//printf("Tracks=%d, Heads=%d, Sectors=%d, SectorSize=%d\n",I,K,N,L);
      if(!K||!N||!L||!I) { ps4LinkClose(F);return(0); }
      /* Create a new disk image */
      if(!NewFDI(D,K,I,N,L)) { ps4LinkClose(F);return(0); }
      ...
      ps4LinkLseek(F,0x100,SEEK_SET);
      /* Read tracks */
      for(I*=K;I;--I)
      {
        /* Read track header */
        if(ps4LinkRead(F,Buf,0x18)!=0x18) break;
        /* Check magic string */
        ...
        DDir[6] = N;
        /* Read sector headers */
        for(DDir+=7,J=N,K=0;J&&(ps4LinkRead(F,Buf,8)==8);DDir+=7,--J,K+=SecSizes[L])
        {
        ...
        if(ps4LinkLseek(F,0x100-0x18-8*N,SEEK_CUR)<0) break;
        /* Read track data */
        if(ps4LinkRead(F,P,K)!=K) break; else P+=K;
      }
      /* Done */
      P = D->Data;
      break;
    ...
    case FMT_IMG: /* If .IMG format... */
      ...
      for(J=0;J<I;++J)
        if(ps4LinkRead(F,P+L*2*(J%K)+(J>=K? L:0),L)!=L) break;
      /* Done */
      P = D->Data;
      break;
    ...
    case FMT_SCL: /* If .SCL format... */
      ...
      /* Read header */
      if(ps4LinkRead(F,Buf,9)!=9) { ps4LinkClose(F);return(0); }
      /* Verify .SCL format tag and the number of files */
      if(memcmp(Buf,"SINCLAIR",8)||(Buf[8]>128)) { ps4LinkClose(F);return(0); }
      /* Create a new disk image */
      P = NewFDI(
            D,
            Formats[Format].Sides,
      ...
      for(J=0,K=D->Sectors,DDir=P;(J<Buf[8])&&(K<I);++J)
      {
        ...
        if(ps4LinkRead(F,DDir,14)!=14) break;
      ...
      if(J<Buf[8]) ps4LinkLseek(F,(Buf[8]-J)*14,SEEK_CUR);
    ...
    case FMT_HOBETA: /* If .$* format... */
      ...
      if(!P) { ps4LinkClose(F);return(0); }
      /* Read header */
      if(ps4LinkRead(F,P,17)!=17) { ps4LinkClose(F);return(0); }
      /* Determine data offset and size */
...
int SaveFDI(FDIDisk *D,const char *FileName,int Format)
{
  ...
  if(!(F=ps4LinkOpen(FileName,O_RDWR,0777))) return(0);

  /* Depending on the format... */
  switch(Format)
  {
    case FMT_FDI:
      if(ps4LinkWrite(F,D->Data,D->DataSize)!=D->DataSize)
      { ps4LinkClose(F);return(0); }
      break;
      ...
      for(I=0;I<Formats[Format].Sides;++I)
        for(J=0;J<Formats[Format].Tracks;++J)
          for(K=0;K<Formats[Format].Sectors;++K)
          {
            ...
            if(!P||(ps4LinkWrite(F,P,C)!=C)) break;
          }
      /* If failed to write all sectors, clean up */
      if(I<Formats[Format].Sides) { ps4LinkClose(F);return(0); }
      break;
      ...
      for(P=FDI_DIR(D->Data);J;--J,P=T)
      {
        ...
        for(I=FDI_SECTORS(P),T=P+7;I;--I,T+=7)
          if(L)
          {
            ...
            K = K>L? L:K;
            L-= K;
            if(ps4LinkWrite(F,FDI_SECTOR(D->Data,P,T),K)!=K)
            { ps4LinkClose(F);return(0); }
          }
        /* Fill remaining track length with zeros */
        if(L>0) ps4LinkLseek(F,L,SEEK_CUR);
      }
      /* Done */
      break;
    ...
    case FMT_SCL:
      ...
      S[8]=T[0x8E4];
      if(ps4LinkWrite(F,S,9)!=9) { ps4LinkClose(F);return(0); }
      for(C=I=0;I<9;++I) C+=S[I];
      /* Write directory entries */
      for(J=0,P=T;J<T[0x8E4];++J,P+=16)
      {
        if(ps4LinkWrite(F,P,14)!=14) { ps4LinkClose(F);return(0); }
      ...
      S[2] = (C>>16)&0xFF;
      S[3] = (C>>24)&0xFF;
      if(ps4LinkWrite(F,S,4)!=4) { ps4LinkClose(F);return(0); }
      /* Done */
      break;
    ...
    case FMT_HOBETA:
      ...
      if(J>=T[0x8E4]) { ps4LinkClose(F);return(0); }      
      /* Copy header */
      memcpy(S,P,14);
      ...
      S[16] = (C>>8)&0xFF;
      /* Write header */
      if(ps4LinkWrite(F,S,17)!=17) { ps4LinkClose(F);return(0); }
      /* Write file data */
      if(ps4LinkWrite(F,P,I)!=I) { ps4LinkClose(F);return(0); }
      /* Done */
      break;
