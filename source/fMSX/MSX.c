...
#endif
#define PRINTOK           if(Verbose) debugNetPrintf(INFO,"OK\n")
#define PRINTFAILED       if(Verbose) debugNetPrintf(INFO,"FAILED\n")
#define PRINTRESULT(R)    if(Verbose) debugNetPrintf(INFO,(R)? "OK\n":"FAILED\n")
#else
...
int  Mode        = MSX_MSX2|MSX_PAL|MSX_GUESSA|MSX_GUESSB;
...
const char *ROMName[MAXCARTS] = { "host0:CARTA.ROM","host0:CARTB.ROM" };
...
const char *DSKName[MAXDRIVES] = { "host0:DRIVEA.DSK","host0:DRIVEB.DSK" };

/** Soundtrack logging ***************************************/
const char *SndName = 0; //"host0:LOG.MID";   /* Sound log file         */

/** Emulation state saving ***********************************/
const char *STAName = "host0:DEFAULT.STA";/* State file (autogen-d)*/

/** Fixed font used by fMSX **********************************/
const char *FNTName = "host0:DEFAULT.FNT"; /* Font file for text   */
...
/** Cassette tape ********************************************/
const char *CasName = 0;//"host0:DEFAULT.CAS";  /* Tape image file     */
//FILE *CasStream;
int CasStream;
/** Serial port **********************************************/
...
int StartMSX(int NewMode,int NewRAMPages,int NewVRAMPages)
{
  ...
  if(*T!=1)
  {
    debugNetPrintf(INFO,"********** This machine is high-endian. **********\n");
    debugNetPrintf(INFO,"Take #define LSB_FIRST out and compile fMSX again.\n");
    return(0);
  }
#else
  if(*T==1)
  {
    debugNetPrintf(INFO,"********* This machine is low-endian. **********\n");
    debugNetPrintf(INFO,"Insert #define LSB_FIRST and compile fMSX again.\n");
    return(0);
  }
#endif
  ...
  if(Verbose) debugNetPrintf(INFO,"Allocating 16kB for empty space...\n");
  ...
  if(!MODEL(MSX_MSX1)&&OPTION(MSX_MSXDOS2)&&(MemMap[3][1][2]!=EmptyRAM)&&!ROMData[2])
    if(LoadCart("host0:system/MSXDOS2.ROM",2,MAP_GEN16))
  ...
  if(!MODEL(MSX_MSX1))
  {
    ...
    if((J<MAXSLOTS)&&LoadCart("host0:system/PAINTER.ROM",J,0)) ++J;
  }

  /* Load FMPAC cartridge */
  for(;(J<MAXSLOTS)&&ROMData[J];++J);
  if((J<MAXSLOTS)&&LoadCart("host0:system/FMPAC.ROM",J,MAP_FMPAC)) ++J;

  /* Load Konami GameMaster2/GameMaster cartridges */
  for(;(J<MAXSLOTS)&&ROMData[J];++J);
  if(J<MAXSLOTS)
  {
    if(LoadCart("host0:system/GMASTER2.ROM",J,MAP_GMASTER2)) ++J;
    else if(LoadCart("host0:system/GMASTER.ROM",J,0)) ++J;
  }
  ...
  if(Verbose)
    debugNetPrintf(INFO,"Redirecting printer output to %s...OK\n",PrnName? PrnName:"STDOUT");
  ...
  {
    if(Verbose) debugNetPrintf(INFO,"Redirecting serial I/O to %s...",ComName);
  ...
  if(CasName&&ChangeTape(CasName))
    if(Verbose) debugNetPrintf(INFO,"Using %s as a tape\n",CasName);
  ...
  for(J=0;J<MAXDRIVES;++J)
  {
    ...
    if(ChangeDisk(J,DSKName[J]))
      if(Verbose) debugNetPrintf(INFO,"Inserting %s into drive %c\n",DSKName[J],J+'A');  
  }
  ...
  if(Verbose)
  {
    debugNetPrintf(INFO,"Initializing VDP, FDC, PSG, OPLL, SCC, and CPU...\n");
    debugNetPrintf(INFO,"  Attached %s to joystick port A\n",JoyTypes[JOYTYPE(0)]);
    debugNetPrintf(INFO,"  Attached %s to joystick port B\n",JoyTypes[JOYTYPE(1)]);
    debugNetPrintf(INFO,"  %d CPU cycles per HBlank\n",HPeriod);
    debugNetPrintf(INFO,"  %d CPU cycles per VBlank\n",VPeriod);
    debugNetPrintf(INFO,"  %d scanlines\n",VPeriod/HPeriod);
  }

  /* Start execution of the code */
  if(Verbose) debugNetPrintf(INFO,"RUNNING ROM CODE...\n");
  A=RunZ80(&CPU);

  /* Exiting emulation... */
  if(Verbose) debugNetPrintf(INFO,"EXITED at PC = %04Xh.\n",A);
  return(1);
}
...
void TrashMSX(void)
{
  ...
  if(SaveCMOS)
  {
    if(Verbose) debugNetPrintf(INFO,"Writing CMOS.ROM...");
    if(!(F=ps4LinkOpen("host0:system/CMOS.ROM",O_RDWR|O_CREAT,0777))) SaveCMOS=0;
    else
    {
      if(ps4LinkWrite(F,RTC,sizeof(RTC))!=sizeof(RTC)) SaveCMOS=0;
      ps4LinkClose(F);
    }
...
{
  debugNetPrintf(INFO,"Reset MSX with mode %x, rampages %d , vrampages %d\n",NewMode,NewRAMPages,NewVRAMPages);
  ...
  if((Mode^NewMode)&MSX_MODEL)
  {
    ...
    switch(NewMode&MSX_MODEL)
    {
      ...
      default:
        ...
        if(Verbose) debugNetPrintf(INFO,"ResetMSX(): INVALID HARDWARE MODEL!\n");
        NewMode=(NewMode&~MSX_MODEL)|(Mode&MSX_MODEL);
        break;
    }
  ...
  if((Mode^NewMode)&MSX_MODEL)
  {
    ...
    if(Verbose) debugNetPrintf(INFO,"  Patching BIOS: ");
    for(J=0;BIOSPatches[J];++J)
    {
      if(Verbose) debugNetPrintf(INFO,"%04X..",BIOSPatches[J]);
  ...
  if((Mode^NewMode)&MSX_PATCHBDOS)
  {
    ...
    else
    {
      ...
      {
        if(Verbose) debugNetPrintf(INFO,"  Patching BDOS: ");
        /* Apply patches to BDOS */
        for(J=0;DiskPatches[J];++J)
        {
          if(Verbose) debugNetPrintf(INFO,"%04X..",DiskPatches[J]);
  ...
  if((NewRAMPages<(MODEL(MSX_MSX1)? 4:32))||(NewRAMPages>256))
    NewRAMPages=MODEL(MSX_MSX1)? 4:32;
...
case 0xA2: /* PSG input port */
  ...
  if(PSG.Latch==15)
  {
    ...
    /*debugNetPrintf(INFO,"Reading from PSG[15]\n");*/
...
case 0xD4: /* FDC IRQ/DRQ */
  ...
  if(Verbose&0x20) debugNetPrintf(INFO,"I/O: Read from unknown PORT[%02Xh]\n",Port);
  return(NORAM);
}
...
case 0xA1: /* PSG Data */
  ...
  {
    /* @@@ For debugging purposes */
    /*debugNetPrintf(INFO,"Writing PSG[15] <- %02Xh\n",Value);*/
...
case 0xFF: /* Mapper page at C000h */
  ...
  {
    if(Verbose&0x08) debugNetPrintf(INFO,"RAM-MAPPER: block %d at %Xh\n",Value,J*0x4000);
    I=J<<1;
    RAMMapper[J]      = Value;
  ...
  /* Unknown port */
  if(Verbose&0x20)
    debugNetPrintf(INFO,"I/O: Write to unknown PORT[%02Xh]=%02Xh\n",Port,Value);
}
...
/* @@@ For debugging purposes
debugNetPrintf(INFO,"(%04Xh) = %02Xh at PC=%04Xh\n",A,V,CPU.PC.W);
*/
  ...
  switch(ROMType[I])
  {
    case MAP_GEN8: /* Generic 8kB cartridges (Konami, etc.) */
      ...
      if(Verbose&0x08)
        debugNetPrintf(INFO,"ROM-MAPPER %c: 8kB ROM page #%d at %d:%d:%04Xh\n",I+'A',V,PS,SS,J*0x2000+0x4000);
      return;
    ...
    case MAP_GEN16: /* Generic 16kB cartridges (MSXDOS2, HoleInOneSpecial) */
      ...
      if(Verbose&0x08)
        debugNetPrintf(INFO,"ROM-MAPPER %c: 16kB ROM page #%d at %d:%d:%04Xh\n",I+'A',V>>1,PS,SS,J*0x2000+0x4000);
      return;
    ...
    case MAP_ASCII8: /* ASCII 8kB cartridges */
      ...
      if((A>=0x6000)&&(A<0x8000))
      {
        ...
        if(V&(ROMMask[I]+1))
        {
          ...
          P=SRAMData[I];
          if(Verbose&0x08)
            debugNetPrintf(INFO,"ROM-MAPPER %c: 8kB SRAM at %d:%d:%04Xh\n",I+'A',PS,SS,J*0x2000+0x4000);
        }
        else
        {
          /* Select ROM page */
    ...
    case MAP_ASCII16: /*** ASCII 16kB cartridges ***/
      ...
      if((A>=0x6000)&&(A<0x8000)&&((V<=ROMMask[I]+1)||!(A&0x0FFF)))
      {
        ...
        if(V&(ROMMask[I]+1))
        {
          ...
          P=SRAMData[I];
          if(Verbose&0x08)
            debugNetPrintf(INFO,"ROM-MAPPER %c: 2kB SRAM at %d:%d:%04Xh\n",I+'A',PS,SS,J*0x2000+0x4000);
        }
        else
        {
          /* Select ROM page */
    ...
    case MAP_GMASTER2: /* Konami GameMaster2+SRAM cartridge */
      ...
      if((A>=0x6000)&&(A<=0xA000)&&!(A&0x1FFF))
      {
        ...
        if(V&0x10)
        {
          ...
          ROMMapper[I][J]=0xFF;
          if(Verbose&0x08)
            debugNetPrintf(INFO,"GMASTER2 %c: 4kB SRAM page #%d at %d:%d:%04Xh\n",I+'A',(V&0x20)>>5,PS,SS,J*0x2000+0x4000);
        }
        else
        {
          ...
          if(Verbose&0x08)
            debugNetPrintf(INFO,"GMASTER2 %c: 8kB ROM page #%d at %d:%d:%04Xh\n",I+'A',V,PS,SS,J*0x2000+0x4000);
        }
        /* Done with page switch */
        return;
      }
    ...
    case MAP_FMPAC: /* Panasonic FMPAC+SRAM cartridge */
      ...
      switch(A)
      {
        case 0x7FF7: /* ROM page select */
          ...
          if(Verbose&0x08)
            debugNetPrintf(INFO,"FMPAC %c: 16kB ROM page #%d at %d:%d:4000h\n",I+'A',V>>1,PS,SS);
          return;
        case 0x7FF6: /* OPL1 enable/disable? */
          if(Verbose&0x08)
            debugNetPrintf(INFO,"FMPAC %c: (7FF6h) = %02Xh\n",I+'A',V);
          V&=0x11;
          return;
        ...
        case 0x5FFF: /* (5FFEh)=4Dh, then write 69h to enable SRAM */
          ...
          RAM[3]=MemMap[PS][SS][3]=P+0x2000;
          if(Verbose&0x08)
            debugNetPrintf(INFO,"FMPAC %c: 8kB SRAM %sabled at %d:%d:4000h\n",I+'A',FMPACKey==FMPAC_MAGIC? "en":"dis",PS,SS);
          return;
      }
      /* Write to SRAM */
  ...
  if(Verbose&0x08) debugNetPrintf(INFO,"MEMORY: Bad write (%d:%d:%04Xh) = %02Xh\n",PS,SS,A,V);
}
...
{
  if(CasStream) ps4LinkClose(CasStream);
  CasStream = FileName? ps4LinkOpen(FileName,O_RDWR|O_CREAT,0777):0;
...
/*************************************************************/
void RewindTape(void) { if(CasStream) ps4LinkLseek(CasStream,0,SEEK_SET); }
...
/** or a disk image, based on the file extension, etc.      **/
/*************************************************************/
int LoadFileDrive(const char *FileName, byte diskdrive)
{
  int J;
...
byte LoadFNT(const char *FileName)
{
 int F;
  ...
  if(!FontBuf) { ps4LinkClose(F);return(0); }
  /* Read font, ignore short reads */
  ps4LinkRead(F,FontBuf,256*8);
  /* Done */
  ps4LinkClose(F);
  }
  return(1);  
...
/*************************************************************/
byte *LoadROMchungo(const char *Name,int Size,byte *Buf)
{
  int F;
  byte *P;
  int J;
 ...
 // if(!(F=ps4LinkOpen(Name,O_RDONLY,0))) return(0);
  ...
  /* Read data */
  if((J=ps4LinkRead(F,P,Size))!=Size)
  {
    if(!Buf) FreeMemory(P);
    ps4LinkClose(F);
    return(0);
...
int FindState(const char *Name)
{
  ...
  {
    /* Try loading state */
    if(Verbose) debugNetPrintf(INFO,"Loading state from %s...",STAName);
    J=LoadSTA(STAName);
    PRINTRESULT(J);
  }
  ...
  //{
    //I=LoadCHT(P);
    //if(I&&Verbose) debugNetPrintf(INFO,"Loaded %d cheats from %s\n",I,P);
    //FreeMemory(P);
  //}
  ...
  //{
    //I=LoadMCF(P);
    //if(I&&Verbose) debugNetPrintf(INFO,"Loaded %d cheat entries from %s\n",I,P);
    //FreeMemory(P);
  //}
  ...
  //{
    //I=LoadPAL(P);
    //if(I&&Verbose) debugNetPrintf(INFO,"Loaded palette from %s\n",P);
    //FreeMemory(P);
  //}
...
byte *LoadROM(const char *Name,int Size,byte *Buf)
{
  ...
  /* Open file */
//  F=fopen(Name,"rb");
  F=ps4LinkOpen(Name,O_RDONLY,0777);
  debugNetPrintf(INFO,"Opening rom file %s return %d\n",Name,F);
  ...
  if(!F)
  {
    ...
    char *ZipName=strdup(Name);
    char *ArchivedName;

    /* Check if Name is in <ZIPFILE>/<ArchiveFile> notation */
    if(!(ArchivedName=strrchr(ZipName,'/'))) { free(ZipName); return(0); }
    *ArchivedName='\0';
    ArchivedName++;

    /* Assume it's a ZIP file and try opening it */
    if(!(ZF=unzOpen(ZipName))) { free(ZipName); return(0); }

    /* Locate the requested file in the ZIP archive */
    if (unzLocateFile(ZF,ArchivedName,0)!=UNZ_OK)
    {
      free(ZipName);
      unzClose(ZF);
      return(0);
  ...
  if(ZF)
  {
    ...
    /* Read */
    J=unzReadCurrentFile(ZF,P,Size);
...
{
	debugNetPrintf(INFO,"In %s name %s slot %d type %d\n",__FUNCTION__,FileName,Slot,Type);
  ...
  {
    /* Open .SAV file */
    if(Verbose) debugNetPrintf(INFO,"Writing %s...\n",SRAMName[Slot]);
   // if(!(F=fopen(SRAMName[Slot],"wb"))) SaveSRAM[Slot]=0;
	F=ps4LinkOpen(SRAMName[Slot],O_WRONLY|O_CREAT,0777);
    if(F<0)
	{ SaveSRAM[Slot]=0;
	}
    else
      ...
      switch(ROMType[Slot])
      {
        ...
        case MAP_FMPAC:
         // if(fwrite(SRAMData[Slot],1,0x2000,F)!=0x2000) SaveSRAM[Slot]=0;
          if(ps4LinkWrite(F,SRAMData[Slot],0x2000)!=0x2000) SaveSRAM[Slot]=0;
		  
          break;
        case MAP_ASCII16:
          //if(fwrite(SRAMData[Slot],1,0x0800,F)!=0x0800) SaveSRAM[Slot]=0;
          if(ps4LinkWrite(F,SRAMData[Slot],0x0800)!=0x0800) SaveSRAM[Slot]=0;
        ...
        case MAP_GMASTER2:
          ...
          if(ps4LinkWrite(F,SRAMData[Slot],0x1000)!=0x1000)        SaveSRAM[Slot]=0;
          if(ps4LinkWrite(F,SRAMData[Slot]+0x2000,0x1000)!=0x1000) SaveSRAM[Slot]=0;
          break;
      }
  ...
  if(!FileName)
  {
    ...
    /* Cartridge ejected */
    if(Verbose) debugNetPrintf(INFO,"Ejected cartridge from slot %c\n",Slot+'A');
    return(0);
  }

  /* Try opening file */
 // F=fopen(FileName,"rb");
  F=ps4LinkOpen(FileName,O_RDONLY,0777);
  debugNetPrintf(INFO,"Open cart file %s return %d\n",FileName,F);
#ifdef MINIZIP
  unzFile ZF=NULL;
  if(!F)
  {
    /* Name is not an actual file: maybe it's a file inside a ZIP file? */
    char *ZipName=strdup(FileName);
    char *ArchivedName;
  ...
  if(F<0) {return(0);}
#endif
  if(Verbose) debugNetPrintf(INFO,"Found %s:\n",FileName);
 ...
 // if(!fseek(F,0,SEEK_END)) Len=ftell(F);
  ...
  //rewind(F);
    ps4LinkLseek(F,0,SEEK_SET);
//	ps4LinkClose(F);
  //  F=ps4LinkOpen(FileName,PSP2_O_RDONLY,0777);
    //debugNetPrintf(INFO,"Open cart again file %s return %d\n",FileName,F);
  ...
  if(ZF)
  {
    ...
    /* Read two bytes */
    C1=C2=0;
    unzReadCurrentFile(ZF,&C1,1);
    unzReadCurrentFile(ZF,&C2,1);
  ...
  if((C1!='A')||(C2!='B'))
  {
    ...
    if(ZF)
    {
      /* There's currently no fseek-type function for libunzip */
      /* Read the unneeded portion into a temp. area */
      unzOpenCurrentFile(ZF);
      if(unzReadCurrentFile(ZF,EmptyRAM,0x4000)==0x4000)
      {
        unzReadCurrentFile(ZF,&C1,1);
        unzReadCurrentFile(ZF,&C2,1);
        ROM64=(C1=='A')&&(C2=='B');
	...
	if(ps4LinkLseek(F,0x4000,SEEK_SET)>=0)
		
    {
      ...
      //C2=fgetc(F);
		...
		debugNetPrintf(INFO,"second C1=%d C2=%d\n",C1,C2);
  ...
  if((Len>=2)&&((C1!='A')||(C2!='B')))
  {
    ...
    if(ZF)
    {
      ...
      for(I=0;I<Len-2;I++)
      {
        if ((C1=unzReadCurrentFile(ZF,EmptyRAM,0x2000))!=0x2000) break;
        C2+=C1;
      }
      ...
      unzCloseCurrentFile(ZF);
      /* Clean up the EmptyRAM! */
      memset(EmptyRAM,NORAM,0x2000);
    }
    else
#endif
    //if(fseek(F,0x2000*(Len-2),SEEK_SET)>=0)
	if(ps4LinkLseek(F,0x2000*(Len-2),SEEK_SET)>=0)
	
    {
      ...
      //C2=fgetc(F);
	    ...
	    ps4LinkRead(F,&C2,1);
		debugNetPrintf(INFO,"third C1=%d C2=%d\n",C1,C2);
  ...
  if((C1!='A')||(C2!='B'))
  {
    if(Verbose) debugNetPrintf(INFO,"  Not a valid cartridge ROM\n");
    return(0);
  }

  if(Verbose) debugNetPrintf(INFO,"  Cartridge %c: ",'A'+Slot);
  ...
  if((Type>=MAP_GUESS)&&(ROMMask[Slot]+1>4))
  {
    ...
    if(Verbose) debugNetPrintf(INFO,"guessed %s..",ROMNames[Type]);
  ...
  if(MAP_SRAM(Type))
  {
    ...
    if(!SRAMData[Slot])
    {
      if(Verbose) debugNetPrintf(INFO,"scratch SRAM..\n");
      SRAMData[Slot]=EmptyRAM;
    }
    else
    {
      if(Verbose) debugNetPrintf(INFO,"got 16kB SRAM..\n");
    ...
    //  if(F=fopen(SRAMName[Slot],"rb"))
	  F=ps4LinkOpen(SRAMName[Slot],O_RDONLY,0777 );
	  if(F>0)
		  
      {
        /* Read SRAM file */
       // Len=fread(SRAMData[Slot],1,0x4000,F);
        Len=ps4LinkRead(F,SRAMData[Slot],0x4000);
		
        //fclose(F);
        ps4LinkClose(F);
		
        /* Print information if needed */
        if(Verbose) debugNetPrintf(INFO,"loaded %d bytes from %s..",Len,SRAMName[Slot]);
  ...
  ResetMSX(Mode,RAMPages,VRAMPages);
  debugNetPrintf(INFO,"after reset\n");
  PRINTOK;
  ...
  if(!Slot||((Slot==1)&&!ROMData[0]))
  {
    ...
    /* If memory for STAName gets allocated... */
    if((STAName=GetMemory(strlen(FileName)+5)))
    {
      /* Copy name */
      strcpy((char *)STAName,FileName);
      /* Find extension */
      P=strrchr(STAName,'.');
      /* Either replace extension with ".sta" or add ".sta" */
      if(P) strcpy(P,".sta"); else strcat((char *)STAName,".sta");
      /* Try loading state */
      if(Verbose) debugNetPrintf(INFO,"Loading state from %s...\n",STAName);
      C1=LoadSTA(STAName);
      PRINTRESULT(C1);
    }
  }
  debugNetPrintf(INFO,"LoadCart return %d\n",Pages);
...
/** in 16kB pages on success, 0 on failure.                 **/
/*************************************************************/
int LoadCartchungo(const char *FileName,int Slot,int Type)
{
  ...
  {
    /* Open .SAV file */
    if(Verbose) debugNetPrintf(INFO,"Writing %s...",SRAMName[Slot]);
	F=ps4LinkOpen(SRAMName[Slot],O_WRONLY|O_CREAT,0777);
    if(F<0) SaveSRAM[Slot]=0;
    else
    {
      /* Write .SAV file */
      switch(ROMType[Slot])
  ...
  if(MAP_SRAM(Type))
  {
    ...
    if((SRAMName[Slot]=(char *)GetMemory(strlen(FileName)+5)))
    {
      ...
      /* Try opening file... */
	  F=ps4LinkOpen(SRAMName[Slot],O_RDONLY,0);
      if(F>0)
      {
        /* Read SRAM file */
        Len=ps4LinkRead(F,SRAMData[Slot],0x4000);
...
int SaveSTA(const char *FileName)
{
  ...
  /* Open state file */
  if(!(F=ps4LinkOpen(FileName,O_RDWR|O_CREAT,0777))) return(0);
  ...
  if(ps4LinkWrite(F,Header,sizeof(Header))!=sizeof(Header))
  { ps4LinkClose(F);return(0); }
  ...
  if(ps4LinkWrite(F,&CPU,sizeof(CPU))!=sizeof(CPU))
  { ps4LinkClose(F);return(0); }
  if(ps4LinkWrite(F,&PPI,sizeof(PPI))!=sizeof(PPI))
  { ps4LinkClose(F);return(0); }
  if(ps4LinkWrite(F,VDP,sizeof(VDP))!=sizeof(VDP))
  { ps4LinkClose(F);return(0); }
  if(ps4LinkWrite(F,VDPStatus,sizeof(VDPStatus))!=sizeof(VDPStatus))
  { ps4LinkClose(F);return(0); }
  if(ps4LinkWrite(F,Palette,sizeof(Palette))!=sizeof(Palette))
  { ps4LinkClose(F);return(0); }
  if(ps4LinkWrite(F,&PSG,sizeof(PSG))!=sizeof(PSG))
  { ps4LinkClose(F);return(0); }
  if(ps4LinkWrite(F,&OPLL,sizeof(OPLL))!=sizeof(OPLL))
  { ps4LinkClose(F);return(0); }
  if(ps4LinkWrite(F,&SCChip,sizeof(SCChip))!=sizeof(SCChip))
  { ps4LinkClose(F);return(0); }
  if(ps4LinkWrite(F,State,sizeof(State))!=sizeof(State))
  { ps4LinkClose(F);return(0); }

  /* Save memory contents */
  if(ps4LinkWrite(F,RAMData,RAMPages*0x4000)!=RAMPages*0x4000)
  { ps4LinkClose(F);return(0); }
  if(ps4LinkWrite(F,VRAM,VRAMPages*0x4000)!=VRAMPages*0x4000)
  { ps4LinkClose(F);return(0); }
...
int LoadSTA(const char *FileName)
{
  ...
  /* Read the header */
  if(ps4LinkRead(F,Header,sizeof(Header))!=sizeof(Header))
  { ps4LinkClose(F);return(0); }
  ...
  if(ps4LinkRead(F,&CPU,sizeof(CPU))!=sizeof(CPU))
  { ps4LinkClose(F);return(0); }
  if(ps4LinkRead(F,&PPI,sizeof(PPI))!=sizeof(PPI))
  { ps4LinkClose(F);return(0); }
  if(ps4LinkRead(F,VDP,sizeof(VDP))!=sizeof(VDP))
  { ps4LinkClose(F);return(0); }
  if(ps4LinkRead(F,VDPStatus,sizeof(VDPStatus))!=sizeof(VDPStatus))
  { ps4LinkClose(F);return(0); }
  if(ps4LinkRead(F,Palette,sizeof(Palette))!=sizeof(Palette))
  { ps4LinkClose(F);return(0); }
  if(ps4LinkRead(F,&PSG,sizeof(PSG))!=sizeof(PSG))
  { ps4LinkClose(F);return(0); }
  if(ps4LinkRead(F,&OPLL,sizeof(OPLL))!=sizeof(OPLL))
  { ps4LinkClose(F);return(0); }
  if(ps4LinkRead(F,&SCChip,sizeof(SCChip))!=sizeof(SCChip))
  { ps4LinkClose(F);return(0); }
  if(ps4LinkRead(F,State,sizeof(State))!=sizeof(State))
  { ps4LinkClose(F);return(0); }

  /* Load memory contents */
  if(ps4LinkRead(F,RAMData,Header[5]*0x4000)!=Header[5]*0x4000)
  { ps4LinkClose(F);return(0); }
  if(ps4LinkRead(F,VRAM,Header[6]*0x4000)!=Header[6]*0x4000)
  { ps4LinkClose(F);return(0); }
