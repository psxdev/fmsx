...
****************************************************************/
{
  if(Verbose&0x04) debugNetPrintf(3,"CHECK DISK %c\n",R->AF.B.h+'A');
...
****************************************************************/
{
  ...
  if(Verbose&0x04) debugNetPrintf(3,"TAPE: Looking for header...");

  R->AF.B.l|=C_FLAG;
  if(CasStream)
  {
    Pos=ps4LinkLseek(CasStream,0,SEEK_CUR);
    if(Pos&7)
      if(ps4LinkLseek(CasStream,8-(Pos&7),SEEK_CUR))
      {
        if(Verbose&0x04) puts("FAILED");
        ps4LinkLseek(CasStream,0,SEEK_SET);return;
      }

    while(ps4LinkRead(CasStream,Buf,8)==8)
    ...
    ps4LinkLseek(CasStream,0,SEEK_SET);
  }
...
****************************************************************/
{
  ...
  if(CasStream)
  {
    ret=ps4LinkRead(CasStream,&J,1);
    if(ret<0) ps4LinkLseek(CasStream,0,SEEK_SET);
...
****************************************************************/
{
  ...
  if(CasStream)
  {
    ...
    ps4LinkWrite(CasStream,TapeHeader,8);
    R->AF.B.l&=~C_FLAG;
  }   
...
****************************************************************/
  ...
  if(CasStream)
  {
    ps4LinkWrite(CasStream,&R->AF.B.h,1);
    R->AF.B.l&=~C_FLAG;
  }
...
default:
  debugNetPrintf(3,"Unknown BIOS trap called at PC=%04Xh\n",R->PC.W-2);
